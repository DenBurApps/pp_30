...
public class GameController : MonoBehaviour
{
    ...
    public void EndGame()
    {
        if(_currentPoints >= _taskPoints)
        {
            ...
            gm.stars[_currentLevelIndex] = 1;
            if(_startBalls - _currentBalls <= 6)
            {
                gm.stars[_currentLevelIndex] = 2;
            }
            if (_startBalls - _currentBalls <= 3)
            {
                gm.stars[_currentLevelIndex] = 3;
            }
            if (_currentLevelIndex < gm.UnlockedLevels)
            {
                gm.UnlockedLevels += 1;
            }
            gm.SaveCompletedlevel(gm.stars[_currentLevelIndex],_currentLevelIndex);
        }
        else
        {
